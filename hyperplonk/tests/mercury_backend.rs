//! HyperPlonk + Mercury backend correctness tests.
//!
//! Run with: cargo test -p hyperplonk --test mercury_backend -- --nocapture
...
mod tests {
    ...
    #[test]
    fn test_hyperplonk_mercury_e2e() -> Result<(), HyperPlonkErrors> {
        let mut rng = test_rng();
        let gates = CustomizedGates::vanilla_plonk_gate();
        let pcs_srs = MercuryPCS::<E>::gen_srs_for_testing(&mut rng, 15)?;
        ...
        for nv in [4usize, 5, 6] {
            ...
            assert!(ok, "HyperPlonk+Mercury verify failed at nv={nv}");
        }
        Ok(())
    }

    #[test]
    fn test_hyperplonk_mercury_rejects_tampered_public_input() -> Result<(), HyperPlonkErrors> {
        let mut rng = test_rng();
        let gates = CustomizedGates::vanilla_plonk_gate();
        let pcs_srs = MercuryPCS::<E>::gen_srs_for_testing(&mut rng, 13)?;
        let nv = 5;
        let size = 1 << nv;
        let circuit = MockCircuit::<FrType>::new(size, &gates);
        let (pk, vk) = <PolyIOP<FrType> as HyperPlonkSNARK<E, MercuryPCS<E>>>::preprocess(
        ...
        if !bad_inputs.is_empty() {
            ...
            let res = <PolyIOP<FrType> as HyperPlonkSNARK<E, MercuryPCS<E>>>::verify(
                &vk,
                &bad_inputs,
                &proof,
            );
