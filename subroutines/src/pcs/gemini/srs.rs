//! Structured Reference String for Gemini PCS.
//!
//! Gemini needs G1 KZG powers up to N = 2^num_vars for prover commitments
...
/// Universal parameters for Gemini PCS. Contains prover G1 powers
/// up to `max_degree` = 2^num_vars, plus verifier G2 basis.
#[derive(CanonicalSerialize, CanonicalDeserialize, Clone, Debug)]
pub struct GeminiUniversalParams<E: Pairing> {
    pub prover_param: GeminiProverParam<E>,
    pub verifier_param: GeminiVerifierParam<E>,
}

/// Prover parameters: G1 powers for MSM.
#[derive(CanonicalSerialize, CanonicalDeserialize, Clone, Debug)]
pub struct GeminiProverParam<E: Pairing> {
...
#[derive(CanonicalSerialize, CanonicalDeserialize, Clone, Debug)]
pub struct GeminiVerifierParam<E: Pairing> {
    pub g: E::G1Affine,
    pub h: E::G2Affine,
    pub h_x: E::G2Affine,
    pub max_degree: usize,
...
impl<E: Pairing> GeminiProverParam<E> {
...
impl<E: Pairing> StructuredReferenceString<E> for GeminiUniversalParams<E> {
    ...
    fn gen_srs_for_testing<R: Rng>(
        ...
        let pp = GeminiProverParam {
            g1_powers,
            max_degree,
            max_num_vars: supported_num_vars,
        };
        let vp = GeminiVerifierParam {
            g: pp.g1_powers[0],
            h: h_affine,
            h_x,
            max_degree,
...
mod tests {
    ...
    #[test]
    fn test_gemini_srs_gen() -> Result<(), PCSError> {
        let mut rng = test_rng();
        for nv in [2, 4, 6] {
            let srs = GeminiUniversalParams::<Bls12_381>::gen_srs_for_testing(&mut rng, nv)?;
            let n = 1 << nv;
    ...
    #[test]
    fn test_gemini_srs_rejects_huge_num_vars_without_panic() {
        let mut rng = test_rng();
        let huge = usize::BITS as usize;
        let r = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            GeminiUniversalParams::<Bls12_381>::gen_srs_for_testing(&mut rng, huge)
        }));
        match r {
